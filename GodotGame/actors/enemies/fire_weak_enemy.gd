...
func _ready() -> void:
	...
	fov_angle = 360.0
	hearing_range = 900.0
	EnemyState["health"] = FIRE_WEAK_HEALTH
	EnemyState["max_health"] = FIRE_WEAK_HEALTH
	if health_bar:
		health_bar.max_value = FIRE_WEAK_HEALTH
		health_bar.value = FIRE_WEAK_HEALTH
...
func _explode(hit_body: Node = null) -> void:
	...
	ParticleService.fire_burst(global_position, 1.45)
	died.emit()
